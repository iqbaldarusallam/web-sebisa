...
if (!SUPABASE_URL || !SUPABASE_KEY) {
  console.error(
    'SUPABASE_URL / SUPABASE_SECRET_KEY belum diisi. Isi di .env.local terlebih dahulu.',
  );
  process.exit(1);
}
...
async function upsertService(service) {
  const url = `${SUPABASE_URL}/rest/v1/services`;

  // Check if service exists
  const checkUrl = `${url}?title=eq.${encodeURIComponent(service.title)}&select=id`;
  ...
  if (existing.length > 0) {
    ...
    if (!updateResponse.ok) {
      ...
      console.error(`Failed to update ${service.title}:`, error);
      return false;
    }
...
async function main() {
  ...
  for (const service of services) {
    const success = await upsertService(service);
    if (success) {
      successCount++;
    } else {
      failCount++;
